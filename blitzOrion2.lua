...
function OrionLib:MakeNotification(NotificationConfig)
    spawn(function()
        ...
        local NotificationFrame = SetChildren(SetProps(MakeElement("RoundFrame", NotificationConfig.BackgroundColor, 0, 10), {
...
local TimerBar = SetChildren(
    SetProps(
        MakeElement("RoundFrame", Color3.fromRGB(0, 200, 255), 0, 4),
        {
    ...
    ),
    {
        ...
        -- ▼ タイマーバーをテキスト下に配置 ▼
        local ContentLabel = NotificationFrame.Content

        -- 自動サイズが反映されるまで待つ
        RunService.Heartbeat:Wait()

        local contentBottomY = ContentLabel.AbsolutePosition.Y + ContentLabel.AbsoluteSize.Y
        ...
        }),
        {
            ...
            SetChildren(
                ...
                }),
                {
                    ...
                    -- Owner/Member ラベル
                    AddThemeObject(SetProps(MakeElement("Label", labelText, 12), {
...
function ElementFunction:AddPlayerParagraph(userId, Description)
    userId = userId or 0
    Description = Description or ""  -- 新しく説明文を追加
    ...
    local ParagraphFrame = AddThemeObject(SetChildren(SetProps(MakeElement("RoundFrame", Color3.fromRGB(255, 255, 255), 0, 5), {
        Size = UDim2.new(1, 0, 0, 90), -- 高さを少し増やして説明文用に余白
        BackgroundTransparency = 0.7,
        Parent = ItemParent
    }), {
        -- アバター画像
    ...
    function PlayerParagraph:Set(newUserId, newDescription)
        newUserId = newUserId or 0
        newDescription = newDescription or Description
        ...
        ParagraphFrame.Description.Text = newDescription
    end
